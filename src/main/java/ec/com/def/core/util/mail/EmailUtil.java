...
import ec.com.def.core.util.enums.EmailSecurityTypeEnum;
import ec.com.def.core.util.enums.EmailStorageTypeEnum;
...
public class EmailUtil {
	...
	public static void main(String args[]) {
		...
		/*EmailDefinition ed = new EmailDefinition.Builder().emailSecurityType(EmailSecurityTypeEnum.SSL)
				.smtpHostServer("imap.gmail.com").port("995").sfPort("995").auth(Boolean.TRUE).password("abadon")
		...
		try {
			...
			ed.setSession( provideSession(ed, EmailSecurityTypeEnum.TLS) );
			sendEmail( ed );
	 ...
	 * @param body
	 */
	public static void sendEmail(EmailDefinition ed) throws DefException {
	...
	private static void sendEmailNoMultipart(EmailDefinition ed) throws DefException {
	...
	private static void sendEmailMultipart(EmailDefinition ed) throws DefException {
	...
	private static BodyPart addImage(EmailDefinition ed) throws DefException {
	...
	private static BodyPart addAttachent(String fileName, byte[] attachment) throws DefException {
	 ...
	 * @throws DefException
	 */
	private static InternetAddress[] generateAddress(List<String> addresses) throws DefException {
		try {
	 ...
	 * @throws DefException
	 */
	private static Store getStorage( EmailDefinition ed, EmailStorageTypeEnum es ) throws DefException{
		try {
			...
			throw new DefException(Constantes.ERROR_CODE_CUSTOM,"ERROR NoSuchProviderException getStorage AL OBTENER LA INFORMACION DE STORA DEL SERVIDOR DE EMAIL" );
		}
	}
	
	private static Message[] getMessagesFromStore(EmailDefinition ed, Store store,Folder emailFolder) throws DefException{
		try {
